...
def run_train(
    ...
    round_trip_cost: float = backtest.DEFAULT_ROUND_TRIP_COST_PCT,
...
def resolve_maker_exit_outcomes(args: argparse.Namespace) -> tuple[str, ...]:
    ...
    Refuses both at once rather than picking a winner: they express the same
    setting, and a run whose reported cost basis is not the one the caller
    thinks they asked for is the failure mode this whole area keeps producing.
    """
    if args.maker_exit and args.maker_exit_outcomes:
        raise SystemExit("pass either --maker-exit or --maker-exit-outcomes, not both")
    if args.maker_exit:
        return tuple(sorted(backtest.EXIT_OUTCOMES))
    if args.maker_exit_outcomes is None:
        # No flag at all means the default execution, which is limit-only:
        # every exit rests. An explicit empty string is how a run asks for
        # taker exits.
        return tuple(sorted(backtest.EXIT_OUTCOMES))
    return tuple(part.strip().upper() for part in args.maker_exit_outcomes.split(",") if part.strip())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train.add_argument(
        "--round-trip-cost",
        type=float,
        default=backtest.DEFAULT_ROUND_TRIP_COST_PCT,
        help="total round-trip cost fraction used by the trading_pnl threshold objective and holdout metrics. Defaults to the backtest's DEFAULT_ROUND_TRIP_COST_PCT (0.0004 = 2 x 0.02%% maker), which is what this account pays: limit orders only, both legs resting, no slippage. Read from the constant rather than a literal because the two drifted apart twice -- 0.0008 vs 0.0016, then a taker default this account never uses. Pricing the taker round trip here (0.0016) rejects every edge between 4 and 16bps, which is most of them: a 45-bar horizon return averages ~0.5bps unconditionally. Override only together with the backtest cost flags, and quote the cost of the legs the run actually executes.",
    )
    ...
    mh_parser.add_argument("--round-trip-cost", type=float, default=backtest.DEFAULT_ROUND_TRIP_COST_PCT, help="round-trip cost used by the trading_pnl threshold objective; defaults to the backtest's DEFAULT_ROUND_TRIP_COST_PCT (0.0004 = 2 x 0.02%% maker, the limit-only round trip) so threshold selection and execution cannot drift apart -- they have twice")
    ...
    ev_parser.add_argument("--fee-rate-per-side", type=float, default=None, help="base per-side TAKER fee fraction (defaults to backtest.DEFAULT_FEE_RATE_PER_SIDE = 0.0006, Bitget VIP0); stress runs multiply it")
    ...
    ev_parser.add_argument("--maker-fill-window", type=int, default=backtest.DEFAULT_MAKER_FILL_WINDOW, help="model MAKER (resting limit) entries, matching a backtest run with --maker-fill-window. Entries save the entry-side slippage and only fill when a later bar returns to the limit; 0 = instant taker fill. Match the backtest or the cost stress answers for a different execution model")
