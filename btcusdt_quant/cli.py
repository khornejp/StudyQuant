...
def build_parser() -> argparse.ArgumentParser:
    ...
    train.add_argument(
        ...
        help="total round-trip cost fraction (fees + slippage, both sides) used by the trading_pnl threshold objective and holdout metrics. Defaults to the backtest's DEFAULT_ROUND_TRIP_COST_PCT (0.0016 = 0.16%% = 2*(0.06%% taker fee + 0.02%% slippage), Bitget VIP0) rather than a literal, because the two drifted apart once already: this was 0.0008 while the backtest charged 0.0016, so threshold selection optimized against half the cost execution pays and admitted signals that lose money after fees. Override only together with the backtest's --fee-rate-per-side/--slippage-rate-per-side, and if the run executes maker legs, quote the cost of THOSE legs.",
    )
    ...
    mh_parser.add_argument("--round-trip-cost", type=float, default=backtest.DEFAULT_ROUND_TRIP_COST_PCT, help="2*(fee+slippage) used by the trading_pnl threshold objective; defaults to the backtest's DEFAULT_ROUND_TRIP_COST_PCT (0.0016, Bitget VIP0 taker) so threshold selection and execution cannot drift apart -- they did, at 0.0008 vs 0.0016")
