...
>
> 결과: **30m 연 +10.59%(1.5x 통과, 2x 탈락), 45m 연 +22.67%(1.5x·2x 모두 통과).**
> 45m이 이 프로젝트에서 **비용 게이트를 전부 통과한 첫 후보**다.
...
1. **45m threshold 민감도 0.36~0.44.** 0.40이 넓은 고원 위인지 뾰족한 점인지 가른다. 고원이면
   후보가 크게 강해지고, 뾰족하면 threshold 과적합을 의심해야 한다. **2025 H2를 쓰기 전에 닫아야 할
   마지막 값싼 검증.**
2. **30m 숏 백테스트.** 학습 완료(`exp_mr30_short`, F1 0.2198, Brier 0.057, ECE 0.0086). 번들 조립
...
**2025 H2 홀드아웃은 아직 깨끗하다.** 45m이 처음으로 게이트를 통과했으므로 쓸 조건이 갖춰지는
중이지만, 1번과 3번을 먼저 닫을 것. 한 번 쓰면 오염된다.
